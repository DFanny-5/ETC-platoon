...
WINDOW_W = 1000
WINDOW_H = 800
size_world = 1500      #Size_world is the size of the road, should be bigger than the road
SCALE = 6.0             # Track scale
TRACK_RAD = 900/SCALE   # Track is heavily morphed circle with this radius
PLAYFIELD = size_world/SCALE  # Game over boundary  
    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            if not tile.road_visited:
                ...
                #self.env.reward += 1000.0/len(self.env.track) #original
...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, verbose=1):
        ...
        self.count = 0 # used to measure how long stay within desire distance
        self.seed()
        ...
        reward_desire_distance = 0 
        self.reward = 0.0
        self.prev_reward = 0.0
        total_reward = 0.0
        eward_action = 0.0
        self.long_term_reward = 0.0#used to count long term reward in one game
        self.life_stay_reward = 0.0 # used to encourage live longer
        self.life_count = 0.0
        
        self.verbose = verbose
        self.time_count = 0.0 ################################count for time played 
        ...
        #self.observation_space = spaces.Box(low = 0, high = 255, shape = (6,1),dtype = np.float32)    ###5 observation, change for upper shape
        self.observation_space = spaces.Box(low = 0, high = 255, shape = (6,1),dtype = np.float32)
    ...
    def step(self, action):
        ...
        for i,car in enumerate(self.car):#############################################
            ...
            self.state_total.append(float(vr_1))
                ...
                #if i == 1:
                    #self.state_total.append(action[1])
                    #self.state_total.append(action[2])
        ...
        for i,car in enumerate(self.car):
            if i ==0: ################################only second vehicle has reward
                ...
                if action is not None: # First step without action, called from reset()
                    ...
                    #self.car.fuel_spent = 0.0#original                    
                    step_reward = self.reward - self.prev_reward
                    
                    self.prev_reward = self.reward
                    #if self.tile_visited_count == len(self.track):##########used to have before
                    #    done = True
            ...
            '''
            if abs(x) > PLAYFIELD or abs(y) > PLAYFIELD:#####change_1
            #    done = True
                        #step_reward = -100
                    #if self.reward < 1000:
                    #    done = True
        ...
        print(self.state_total)
        self.long_term_reward += step_reward #used to update long term reward
        return self.state_total, step_reward, done, {}
        #return self.state, step_reward, done, {} #original
    ...
    def render(self, mode='human'):
        ...
        for i,car in enumerate(self.car):
            ...
            vel = car.hull.linearVelocity   ###########################################################################3
        #scroll_x = self.car.hull.position[0]#original
        #scroll_y = self.car.hull.position[1]#original
        #angle = -self.car.hull.angle#original
        #vel = self.car.hull.linearVelocity#original
    ...
    def render_road(self):
        ...
        gl.glVertex3f(+PLAYFIELD+500, +PLAYFIELD, 0)
        gl.glVertex3f(+PLAYFIELD+500, -PLAYFIELD, 0)
        gl.glVertex3f(-PLAYFIELD, -PLAYFIELD, 0)
